...
if not os.path.isdir(case):
    os.makedirs(case) # create a path
...
async def install(lightning):
    if lightning.fwd_from:# Kang with credit you gey
...
async def install(lightning):
    ...
    await lightning.delete() 
        ...
        return
    a = lightning.get_reply_message()
    code = await bot.download_media(a, case)
    openn = open(a, "r")
    openn.readline()
    pygments.highlight(f"{openn}", Python3Lexer(), ImageFormatter(font_name="DejaVu Sans Mono", line_numbers=True), "yo.png")
    await lightning.client.send_file(lightning.chat_id, "out.png", force_document=False, reply_to=event.reply_to_msg_id)
    if lightning.reply_to_msg_id:
        try:
            downloaded_file_name = (
                await lightning.client.download_media(  # pylint:disable=E0602
